...
## Packaging changes for release after a Pull Request

1. Bump the version in `__init__.py` (eg. from v0.0.6 to v0.0.7)
2. Create a release with the exact same version number (v0.0.7); note that the release tag must start with "v" to 
trigger the release pipeline.
3. Pypi will automatically receive the updated package. 
